# Oblique Strategies Version 3.10
...
# Rhythm Blocks & An Empty List For Patterns
...
for step in boogie:
    wut=random.randint(0,choice.steps-1)
    RndPattern.pitch=moodi[wut]
    RndPattern.duration=step
    NOTE=RndPattern(moodi[wut],step)
...
class MIDIPattern:
    def __init__(self,region,midinotes,color):
        self.region=region
...
for reg in ["Subsonic"]:
    ...
    for i in RandomMotif:
        sound=i.pitch
        Subsonic.append(midinotes[sound])
    MIDIPattern.midinotes=Subsonic

    for hue in colors:
        Subcolors.append(midinotes[hue])
    MIDIPattern.color=Subcolors

    kb=MIDIPattern(str(reg),Subsonic,Subcolors)
    regions.append(kb)
...
for reg in ["Basso Ostinato C1 to B1"]:
    ...
    for hue in colors:
        BassoColors.append(midinotes[hue])
    MIDIPattern.color=BassoColors

    kb=MIDIPattern(str(reg),BassoOstinato,BassoColors)
    regions.append(kb)
...
for reg in ["Double Bass Range C2 to B2"]:
    ...
    for hue in colors:
        DoubleBSColors.append(midinotes[hue])
    MIDIPattern.color=DoubleBSColors

    kb=MIDIPattern(str(reg),DoubleBass,DoubleBSColors)
    regions.append(kb)
...
for reg in ["Viola/Cello Range C3 to B3"]:
    ...
    for hue in colors:
        CelloColors.append(midinotes[hue])
    MIDIPattern.color=CelloColors

    kb=MIDIPattern(str(reg),ViolaCello,CelloColors)
    regions.append(kb)

for reg in ["Melody Range from C4 to B4"]:
    MIDIPattern.region=str(reg)
    ...
    for hue in colors:
        MidColors.append(midinotes[hue])
    MIDIPattern.color=MidColors

    kb=MIDIPattern(str(reg),MiddleC,MidColors)
    regions.append(kb)
...
for reg in ["Melody Range from C5 to B5"]:
    ...
    for i in RandomMotif:
        sound=i.pitch
        HighRegister.append(midinotes[sound])
    MIDIPattern.midinotes=HighRegister

    for hue in colors:
        Highcolors.append(midinotes[hue])
    MIDIPattern.color=Highcolors

    kb=MIDIPattern(str(reg),HighRegister,Highcolors)
    regions.append(kb)
...
for reg in ["Overtones C6 to B6"]:
    ...
    for hue in colors:
        Overtonecolors.append(midinotes[hue])
    MIDIPattern.color=Overtonecolors

    kb=MIDIPattern(str(reg),Overtones,Overtonecolors)
    regions.append(kb)

for reg in ["Canine Boogie C7 to B7"]:
    MIDIPattern.region=str(reg)
    ...
    for hue in colors:
        CanineColors.append(midinotes[hue])
    MIDIPattern.color=CanineColors

    kb=MIDIPattern(str(reg),CanineBoogie,CanineColors)
    regions.append(kb)
...
for reg in ["Low Quarter"]:
    ...
    MIDIPattern.color=Subcolors+BassoColors

    kb=MIDIPattern(str(reg),LowQ,Subcolors+BassoColors)
    regions.append(kb)
...
for reg in ["Low to Midland"]:
    ...
    MIDIPattern.color=DoubleBSColors+CelloColors

    kb=MIDIPattern(str(reg),LowMid,DoubleBSColors+CelloColors)
    regions.append(kb)

for reg in ["From Mid to Hi"]:
    MIDIPattern.region=str(reg)
    MidHigh=MiddleC+HighRegister
    MIDIPattern.midinotes=MidHigh
    MIDIPattern.color=MidColors+Highcolors

    kb=MIDIPattern(str(reg),MidHigh,MidColors+Highcolors)
    regions.append(kb)

for reg in ["Canine Overtones"]:
    MIDIPattern.region=str(reg)
    CaniOver=Overtones+CanineBoogie
    MIDIPattern.midinotes=CaniOver
    MIDIPattern.color=Overtonecolors+CanineColors

    kb=MIDIPattern(str(reg),CaniOver,Overtonecolors+CanineColors)
    regions.append(kb)

# Select Keyboard Range

selektio=random.randint(0,len(regions)-1)
KeyboardRange=regions[selektio]
print("")
print("in the Keyboard Range:",KeyboardRange.region)
...
for beat in boogie:
    pitch=KeyboardRange.midinotes[nn]
    duration=beat
...
# Create a Two-Bar MIDI Motif & Add Color Notes (First/Last)

TwoBarSequence=KeyboardRange.midinotes*2
TwoBarSequence[0]=KeyboardRange.color[0]
TwoBarSequence[-1]=random.choice(KeyboardRange.color)
...
mf=MIDIFile(1)
track=0;time=0;channel=0;
np=0

name2="Two-Bar "+ChoosePattern.meter+"-Riff in "+root+" "+choice.mode
mf.addTrackName(track,time,name2)
newname2=name2.replace("/","_")

mf.addTempo(track,time,tracktempo)

for blast in boogie2:
    pitch=TwoBarSequence[np]
    duration=blast
    volume=random.choices([0,100],weights=(15,85),k=1)[0]
    mf.addNote(track,channel,pitch,time,duration,volume)
    time=time+duration
    np=np+1

with open(newname2+".mid","wb") as outf:
    mf.writeFile(outf)
