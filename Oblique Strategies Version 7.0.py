...
# Oblique Strategies Version 7.0
...
for r in ["Symmetrical Augmented"]:
    Scale.mode=str(r)
    SymmetricalAugSteps=[0,3,4,7,8,11]
...
for r in ["Locrian/Aeolian Mixed"]:
    Scale.mode=str(r)
    LocriaeolianSteps=[0,1,2,3,5,6,7,8,10]
    LocriaeolianProbs=[10,15,10,10,5,15,10,10,15]
    Locriaeolian=[]
    for j in LocriaeolianSteps:
        note=notepool[j+transpose]
        Locriaeolian.append(note)
    Scale.notes=Locriaeolian
    Scale.steps=9
    Scale.probmatrix=LocriaeolianProbs
    mode=Scale(str(r),Locriaeolian,9,LocriaeolianProbs)
    modes.append(mode)
...
Short=[[1],[0.5,0.5],[0.75,0.25],[0.25,0.75],[0.25,0.25,0.25,0.25]]
Long=[[1,0.5],[0.5,1],[0.5,0.5,0.5],[1.5],[0.75,0.75],
      [0.25,0.25,1],[0.25,1,0.25],[0.25,0.5,0.5,0.25],
...
for time in ["13/4"]:
    ...
    for s in ["Abbarum 'Aqi"]:
        Rhythm.style=str(s)

        pattern134=[2,1,1,2,2,2,1,1,1]
        Rhythm.sequence=pattern134
        pattern=Rhythm(str(time),str(s),pattern134)
        thirteenfours.append(pattern)

    for s in ["Dirty Dozen"]:
        Rhythm.style=str(s)
...
for time in ["14/4"]:
    ...
    for s in ["This Modern Love"]:
        Rhythm.style=str(s)

        pattern144=[2,2,2,2,3,3]
        Rhythm.sequence=pattern144
        pattern=Rhythm(str(time),str(s),pattern144)
        fourteenfours.append(pattern)
...
for time in ["16/4"]:
    ...
    for s in ["Sammahkum 'Aqi"]:
        Rhythm.style=str(s)

        pattern164=[1,1,2,1,1,2,2,2,2,2]
        Rhythm.sequence=pattern164
        pattern=Rhythm(str(time),str(s),pattern164)
        sixteenfours.append(pattern)

    for s in ["Sweet Sixteens"]:
        Rhythm.style=str(s)
...
for time in ["12/8"]:
    ...
    for s in ["Barber-Shop Doo-Wop"]:
        Rhythm.style=str(s)

        pattern128=[0.5,0.5,0.5,1,0.5,0.5,0.5,0.5,1,0.5]
        Rhythm.sequence=pattern128
        pattern=Rhythm(str(time),str(s),pattern128)
        twelveights.append(pattern)      
...
for time in ["17/8"]:
    ...
    for s in ["Inverted Iqa' Khosh Rang"]:
        Rhythm.style=str(s)

        pattern178=[0.5,1,0.5,0.5,1,0.5,1,0.5,0.5,1,0.5,1]
        Rhythm.sequence=pattern178
        pattern=Rhythm(str(time),str(s),pattern178)
        seventeeneights.append(pattern)
...
st=len(twofours)+len(threefours)+len(fourfours)+len(fivefours)
st2=len(sixfours)+len(sevenfours)+len(ninefours)+len(tenfours)
st3=len(twelvefours)+len(thirteenfours)+len(fourteenfours)
st4=len(sixteenfours)+len(nineteenfours)+len(twentyfours)
be=len(fiveights)+len(sixeights)+len(seveneights)+len(nineights)
be2=len(teneights)+len(eleveneights)+len(twelveights)
be3=len(thirteeneights)+len(fifteeneights)+len(seventeeneights)+len(twentyoneights)

total=st+st2+st3+st4+be+be2+be3
#print("Rhythm Sequences in total:",total)
...
modulations={
    ...
    "Locrian/Aeolian Mixed":[Locrian,Aeolian,Phrygian],
    }
